...
import com.security.utils.TimeUtils;
...
public class JedisKeyValueServiceImpl implements KeyValueService{
    ...
    public void putObj(String key, int expire, Object obj, int dbIndex) {
        try {
            ...
            log.error("redis parse object 2 json fail, error :", e);
        ...
        try (Jedis jedis = jedisPool.getResource()) {
            ...
            if (!CollectionUtils.isEmpty(members)) {
                resultSet.addAll(members.stream()
                        .map(member -> {
                            try {
                                return new ObjectMapper().readValue(member, tClass);
                            } catch (IOException e) {
                                return null;
                            }
        ...
        try (Jedis jedis = jedisPool.getResource()) {
            if (!jedis.exists(key)) {
                ...
                try {
                    ...
                    log.error("transform element to json pattern  fail, reason:", e);
                }
            }
        }
    }
